...
while (i < supplyChanges.length) {
  ...
  if (supplyChanges[i] < 0) {
    console.log('Parts count', supplyChanges[i]);
  }
  i++;
}
...
for (let i = 0; i < supplyChanges.length; i++) {
  partsAvailable= partsAvailable + supplyChanges[i];
}
console.log('9. Total supplies available is:', partsAvailable);
